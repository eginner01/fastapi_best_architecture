...
class Instance(Base):
    ...
    instance_no: Mapped[str] = mapped_column(
        sa.String(50), unique=True, index=True, default='', comment='流程实例编号，例如 INST_202501190001'
    )
    ...
    title: Mapped[str] = mapped_column(sa.String(255), default='', comment='审批实例标题，例如 张三的请假申请')
    ...
    form_data: Mapped[str | None] = mapped_column(sa.JSON, default=None, comment='表单提交数据（JSON格式）')
    ...
    tags: Mapped[str | None] = mapped_column(sa.JSON, default=None, comment='标签（JSON数组）')
    attachments: Mapped[str | None] = mapped_column(sa.JSON, default=None, comment='附件信息（JSON数组）')
    settings: Mapped[str | None] = mapped_column(sa.JSON, default=None, comment='实例配置（JSON格式）')
