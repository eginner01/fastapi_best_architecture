...
CREATE TABLE IF NOT EXISTS `approval_step` (
    ...
    `status` VARCHAR(20) DEFAULT 'PENDING' COMMENT '步骤状态: PENDING-待处理, PROCESSING-处理中, COMPLETED-已完成, CANCELLED-已取消',
    `action` VARCHAR(20) COMMENT '处理动作: APPROVE-同意, REJECT-拒绝, DELEGATE-转交, RETURN-退回',
